...
napi_status checkArgs(napi_env env, napi_callback_info info, char* methodName,
  ...
  if (realArgc != argc) {
    char errorMsg[100];
    snprintf(errorMsg, 100, "For method %s, expected %zi arguments and got %zi.",
  ...
  for ( size_t x = 0 ; x < argc ; x++ ) {
    ...
    if (t != types[x]) {
      char errorMsg[100];
      snprintf(errorMsg, 100, "For method %s argument %zu, expected type %s and got %s.",
...
int32_t rejectStatus(napi_env env, carrier* c, char* file, int32_t line) {
  if (c->status != MACADAM_SUCCESS) {
    ...
    snprintf(errorCodeChars, 20, "%d", c->status);
